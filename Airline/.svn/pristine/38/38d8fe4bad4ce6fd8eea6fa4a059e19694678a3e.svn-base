...
public class GateInfoDatabase extends Observable {
    ...
    /**
     * Constructor for GateInfoDatabase
     * Also creates Gates
     */
    public GateInfoDatabase() {
        gates = new Gate[maxGateNumber + 1];
    ...
    public int getStatus(int gateNumber) {
        return this.gates[gateNumber].getStatus();
    }
    
    /**
     * Returns the mCode of a Gate, given the gateNumber
     */
    ...
    public String[] getStatuses() {
        String[] gateStatuses = new String[this.maxGateNumber];
        for (int i = 0; i < this.maxGateNumber; i++) {
            gateStatuses[i] = this.gates[i].getStatusString();
        }
        return gateStatuses;
    }
    ...
    public void allocate(int gateNumber, int mCode) {
        this.gates[gateNumber].allocate(mCode);
        setChanged();
        notifyObservers();
    }
    ...
    public void departed(int gateNumber) {
        this.gates[gateNumber].departed();
        setChanged();
        notifyObservers();
    }

    /**
     * Gets the Status string of a given Gate based on gateNumber
     */
