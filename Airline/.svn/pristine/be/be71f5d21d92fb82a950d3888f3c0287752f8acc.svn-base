...
public class GOC extends JFrame implements Observer, ActionListener {
    ...
    /**
     * Constructor for GOC. Gets DBs and adds observers.
    ...
    public void actionPerformed(ActionEvent evn) {
        ...
        if (evn.getSource() == nextStateButton) {
            ...
            if (flightCodeList.getSelectedIndex() != -1) {
            	...
            	checkAndChangeStatus(mCode,statusCode);//Checks if Status change applies then changes status
    ...
    /**
     * Finds out if status change applies to GOC and acts accordingly (changes status of MR and/or Gate (or neither))
     * @param mCode
     * @param statusCode
     */
    private void checkAndChangeStatus(int mCode, int statusCode) {
    	if (statusCode == ManagementRecord.WANTING_TO_LAND) {	            	
        	...
        	setTaxiingAndReserveGate(mCode);//Method changes the MR's status to Taxing handles assigning of gate            
    ...
    /**
     * Given an mCode, the method checks that a Gate is selected then Reserves the gate selected.
     * @param mCode
     */
    private void setTaxiingAndReserveGate(int mCode) {
    	if (gateList.getSelectedIndex() != -1) {
        	int gateNumber = gateList.getSelectedIndex();
        	
            if (gateInfoDatabase.getStatus(gateNumber) != Gate.FREE) {
            	JOptionPane.showMessageDialog(this, "Gate reserved");
            } else {
    ...
    /**
     * Called from update method, it fetches the relevant mCodes than displays their flight codes with statuses
     */
    ...
    /**
     * Called from the update method, it creates an array of Gates and displays them.
     */
    private void updateGates() {

        String[] outputArr = new String[gateInfoDatabase.maxGateNumber];
        String[] statuses = gateInfoDatabase.getStatuses();

        for (int i = 0; i < gateInfoDatabase.maxGateNumber; i++) {
            int gateNumber = i + 1;
            outputArr[i] = "Gate " + gateNumber + " - " + statuses[i];
        }
    ...
    /**
     * Gets all mCodes that have a status relevant to GOC
     * @return ArrayList<Integer> the mCodes relevant to GOC
     */
