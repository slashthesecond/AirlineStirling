 ...
 */
   ...
   * Also creates ManagementRecords
   */
 ...
 */
  public Integer[] getWithStatus(int statusCode){
	   Integer[] withStatus = new Integer[this.getStatusNumber(statusCode)];
	   int index = 0;
	    for(int mCode = 0; mCode < MRs.length; mCode++){
	      if(MRs[mCode].getStatus() == statusCode){
	        withStatus[index] = mCode;
	        index++;
	      }
	    }
	    return withStatus;
  ...
  /**
  	Helper method to determine the size of the withStatus array
   */
  private int getStatusNumber(int statusCode){
	    int count = 0;
	    for(ManagementRecord m : MRs){
	      if(m.getStatus() == statusCode){
	        count++;
	      }
	    }
	    return count;
 ...
 * This operation finds a currently FREE MR and forwards the radarDetect request to it for recording.*/
  public boolean radarDetect(FlightDescriptor fd){
	  for(ManagementRecord m: MRs) {
		if(m.getStatus() == ManagementRecord.FREE) {
			m.radarDetect(fd);
			setChanged();
			notifyObservers();
			return true;
 ...
 *  The given passenger is boarding the aircraft with the given mCode. Forward the message to the given MR for recording in the passenger list.*/
  public boolean addPassenger(int mCode, PassengerDetails details){
	  boolean success = MRs[mCode].addPassenger(details);
	  setChanged();
	  notifyObservers();
	  return success;
 ...
 *  Return the Itinerary of the aircraft with the given mCode.*/
  ...
  /**
   * Returns a ManagementRecords's details as a String, given an mCode
   */
  public String getRecordDetails(int mCode) {
	  return MRs[mCode].getDetails();
  }
  
  /**
   * Returns a ManagementRecord's status string (e.g. "Free", "In transit" etc.), given an mCode
   */
  ...
  /**
   * Returns the Gate number that a ManagementRecord is assigned to, given the mCode
   */
  ...
  /**
   * Clears a ManagementRecord's fault description. To be used once the aircraft is repaired.
   */
  public void repair(int mCode) {
  	MRs[mCode].repair();
  }
}
